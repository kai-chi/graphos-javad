...
cmake_minimum_required(VERSION 3.5) # target_include_directories
...
find_path(SGXSSL_LIBRARY_DIR libsgx_tsgxssld.a "${SGXSSL_LIBRARY_PATH}" NO_DEFAULT_PATH)
...
if(SGX_FOUND)
    ...
    function(_build_edl_obj edl edl_search_paths use_prefix use_sgxssl)
    ...
    function(add_trusted_library target)
        ...
        if("${SGX_EDL}" STREQUAL "")
            ...
            _build_edl_obj(${SGX_EDL} "${SGX_EDL_SEARCH_PATHS}" ${SGX_USE_PREFIX} ${SGX_USE_SGXSSL})
        ...
        target_include_directories(${target} PRIVATE ${CMAKE_CURRENT_BINARY_DIR} ${ENCLAVE_INC_DIRS} ${SGXSSL_INCLUDE_PATH})
        ...
        if(${SGX_USE_SGXSSL})
            ...
            target_link_libraries(${target} "${SGX_COMMON_CFLAGS} \
                ...
                -Wl,--start-group -lsgx_tsgxssld -lsgx_tsgxssl_cryptod -lsgx_tstdc -lsgx_pthread -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
    ...
    function(add_enclave_library target)
        ...
        if(${SGX_USE_SGXSSL})
            ...
            target_link_libraries(${target} "${SGX_COMMON_CFLAGS} \
                ...
                -Wl,--start-group ${TLIB_LIST} -lsgx_tsgxssld -lsgx_tsgxssl_cryptod -lsgx_tstdc -lsgx_pthread -lsgx_tcxx -lsgx_tkey_exchange -lsgx_tcrypto -l${SGX_TSVC_LIB} -Wl,--end-group \
                -Wl,-Bstatic -Wl,-Bsymbolic -Wl,--no-undefined \
                -Wl,-pie,-eenclave_entry -Wl,--export-dynamic \
                ${LDSCRIPT_FLAG} \
                -Wl,--defsym,__ImageBase=0")
    ...
    function(add_untrusted_executable target)
        ...
        target_include_directories(${target} PRIVATE ${CMAKE_CURRENT_BINARY_DIR} ${APP_INC_DIRS})
        if (${SGX_USE_SGXSSL})
            list( APPEND CMAKE_INSTALL_RPATH ${CMAKE_CURRENT_SOURCE_DIR})
            list( APPEND CMAKE_INSTALL_RPATH ${CMAKE_CURRENT_SOURCE_DIR}/sample_libcrypto)
            target_link_libraries(${target} "${SGX_COMMON_CFLAGS} \
